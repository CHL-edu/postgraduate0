...
class Config:
    def __init__(self, config_file=None):
        ...
        self.lambda_rate = 0.5  # 泊松过程到达率（每单位时间）
        self.mu_rate = 0.2  # 泊松过程离开率（每单位时间）
        self.simulation_time = 100  # 模拟总时间
...
# 1. 生成光网络拓扑
def generate_topology(config: Config) -> nx.Graph:
    G = nx.Graph()
...
def generate_dynamic_demands(config: Config) -> List[Tuple[int, int, int, float, float]]:
    ...
    return sorted(demands, key=lambda x: x[3])  # 按到达时间排序
...
def visualize_results(G: nx.Graph, rsa_results: List[Dict], config: Config,
    ...
    if show_spectrum:
        ...
        for u, v in edge_list:
            slot_usage.append(G[u][v]['slots'])

        im = ax2.imshow(slot_usage, cmap='viridis', aspect='auto')
        ax2.set_title("Spectrum slot occupancy situation")
        ...
        plt.colorbar(im, ax=ax2, label='Occupation status (0=idle, 1=occupied)')
