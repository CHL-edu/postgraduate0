...
fp = open('/data/chl/pyproject/DeepRSA/Src_Dst_Paths.dat', 'rb')
...
# Create global episodes counter
...
# 确保主网络已经初始化（这会在AC_Net的构造函数中自动完成）
print(f"Master network initialized. Policy variables: {len(master_network.get_policy_variables())}, "
      f"Value variables: {len(master_network.get_value_variables())}")

# Create checkpoint for saving
checkpoint = tf.train.Checkpoint(optimizer=trainer, model=master_network, global_episodes=global_episodes)
checkpoint_manager = tf.train.CheckpointManager(checkpoint, model_path, max_to_keep=5)
...
for i in range(num_agents):
    agents.append(
        DeepRMSA_Agent(i, trainer, linkmap, LINK_NUM, NODE_NUM, SLOT_TOTAL, k_path, M, lambda_req, lambda_time,
                       len_lambda_time, gamma, episode_size, batch_size, Src_Dest_Pair, Candidate_Paths,
                       num_src_dest_pair, model_path, global_episodes, regu_scalar, x_dim_p, x_dim_v,
                       n_actions, num_layers, layer_size, model2_flag, nonuniform, prob_arr))
