...
def setup_logging(output_dir: str):
    ...
    # 创建日志文件路径
    log_file = os.path.join(log_dir, f'training_{time():.0f}.log')
...
def load_candidate_paths(path_file: str, NODE_NUM: int, N: int, logger) -> Dict:
    """加载候选路径 - 改进的错误处理"""
...
def setup_environment(config: Config, logger) -> None:
    """设置环境和创建输出目录"""
    directories = [
        config.output_dir,  # 先创建主输出目录
        config.model_path,
...
class ResourceMonitor:
    ...
    def check_memory(self) -> Tuple[bool, float]:
        ...
        try:
            ...
            memory_percent = memory_gb / self.memory_limit_gb * 100

            # 多级警告
            if memory_percent > 90:
                self.memory_warnings += 1
                if self.memory_warnings >= 3:  # 连续3次超过90%才停止
                    return False, memory_gb
            else:
                self.memory_warnings = 0  # 重置警告计数

            return True, memory_gb
        except Exception as e:
            # 如果无法获取内存信息，继续训练但记录错误
            return True, 0.0
    ...
    def get_stats(self) -> Dict:
        ...
        try:
            return {
                ...
                'runtime_hours': (time() - self.start_time) / 3600
            }
        except Exception:
            return {
                'cpu_percent': 0,
...
def validate_config(config: Config, logger) -> bool:
    ...
    try:
        ...
        logger.error(f"无法写入输出目录 {config.output_dir}: {e}")
        return False
...
def main():
    ...
    try:
        ...
        # 保存配置副本
        config_file_path = os.path.join(config.output_dir, 'training_config.ini')
        config_manager.save_config(config_file_path)
        logger.info(f"✅ 配置已保存至: {config_file_path}")

        # 检查必要的模块
        try:
            from ACNet627 import AC_Net
            from Agent627 import DeepRMSA_Agent
        except ImportError as e:
            logger.error(f"❌ 无法导入必要的模块: {e}")
            logger.error("请确保 ACNet627.py 和 Agent627.py 文件存在且在正确的路径中")
            return
        ...
        # 加载候选路径
        Candidate_Paths = load_candidate_paths(config.candidate_paths_file, config.NODE_NUM, config.N, logger)
        ...
        # 监控循环
        monitor_interval = 30  # 每30秒检查一次（更频繁的检查）
            ...
            try:
                ...
                if current_time - last_monitor_time > monitor_interval:
                    ...
                    if not memory_ok:
                        ...
                        logger.error(f"   连续警告次数: {resource_monitor.memory_warnings}")
                        coord.request_stop()
                        break
                    ...
                    logger.info(f"📊 系统资源: CPU {stats['cpu_percent']:.1f}%, "
                                ...
                                f"线程数 {stats['num_threads']}, "
                                f"运行时间 {stats['runtime_hours']:.2f} 小时")
        ...
        # 保存训练统计
        if 'monitor' in locals() and monitor and 'config' in locals():
